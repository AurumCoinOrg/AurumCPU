...
When the `--proxy` flag is used, `aurumcpud` sends all external traffic via the
...
is made or until `aurumcpud` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `aurumcpu-wallet-cli` and
`aurumcpu-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
AurumCPUd does not support synchronizing the blockchain over onion or I2P hidden services.
You may sync the blockchain using a SOCKS4 proxy. AurumCPUd will connect to IPv4
...
`--tx-proxy` which tells `aurumcpud` the ip/port of a socks proxy provided by a
...
which tells `aurumcpud` to connect to ".onion" P2P addresses using a socks
...
`--anonymous-inbound`. This option tells `aurumcpud` the inbound address, network
...
which tells `aurumcpud` that a max of 25 inbound Tor connections are being
received at address "5tymba6faziy36md5ffy42vatbjzlye4vyr3gyz6lcvdfximnvwpmwqd.onion:18084" and forwarded to `aurumcpud`
...
forwarded to `aurumcpud` localhost port 18085. Using `tx-proxy`(required), these
...
`aurumcpud` RPC port - which is independent from the configuration for incoming
...
I2P implementation.  
You can find guides for i2pd [here](https://docs.getmonero.org/running-node/aurumcpud-tori2p/#__tabbed_1_2).

## Example Node Configurations

Please check the [`aurumcpud` reference](https://docs.getmonero.org/interacting/aurumcpud-reference/#tori2p-and-proxies) for more information about these flags.
...
AurumCPUd will connect to IPv4 nodes via clearnet, revealing to your ISP
...
aurumcpud --tx-proxy tor,127.0.0.1:9050,10
```

### Connect To IPv4 Nodes Over Tor Only

This configuration does not connect to hidden services or accept incoming
connections. Your ISP will see that you are running Tor, but not AurumCPUd.
...
Your ISP will see that you are running Tor and I2P, but not AurumCPUd. Transactions
will be relayed to hidden services. Your node will not accept any incoming
connections (including from Tor and I2P).

```bash
aurumcpud --proxy 127.0.0.1:9050 \
...
You will need to configure [hidden services manually for Tor and I2P](https://docs.getmonero.org/running-node/aurumcpud-tori2p/#node-configuration).
...
This configuration is not currently supported. AurumCPUd relies on IPv4 to sync
...
If a user only runs `aurumcpud` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `aurumcpud` as often as possible to conceal when transactions are being sent.
...
`aurumcpud` currently selects two outgoing connections every 5 minutes for
...
`aurumcpud` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `aurumcpud` should likely disconnect
