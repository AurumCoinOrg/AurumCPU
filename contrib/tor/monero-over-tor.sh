...
for dir in \
  ...
  "$DIR/../../build/Windows/master/release/bin"
do
  if test -x "$dir/aurumcpud"
  then
    aurumcpud="$dir/aurumcpud"
    break
  fi
done
if test -z "$aurumcpud"
then
  echo "aurumcpud not found"
  exit 1
fi
echo "Found: $aurumcpud"
